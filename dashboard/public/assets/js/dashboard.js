...
function createDashboardServer() {
  ...
  api.get('/fivem/bans', async (req, res) => {
    try {
      ...
      // Normalise l'URL (retire le slash final) et construit l'endpoint des bans.
      const baseUrl = rows[0].fivem_url.trim().replace(/\/+$/, '');
      const bansUrl = `${baseUrl}/bans.json`;

      let response;
      try {
        response = await fetch(bansUrl, { signal: AbortSignal.timeout(8000) });
      } catch (fetchErr) {
        console.error('Erreur fetch bans FiveM :', fetchErr);
        return res.status(502).json({
          error: "Impossible de joindre le serveur FiveM. Vérifie que l'URL est correcte et que l'endpoint /bans.json est accessible publiquement."
        });
      }

      if (!response.ok) {
        return res.status(502).json({
          error: `Le serveur FiveM a répondu avec une erreur (HTTP ${response.status}). Vérifie que /bans.json existe bien sur ce serveur.`
        });
      }
      ...
      try {
        ...
        return res.status(502).json({
          error: "La réponse du serveur FiveM n'est pas un JSON valide."
        });
      }

      // Certains systèmes (ex: txAdmin, resources custom) renvoient { bans: [...] } au lieu d'un tableau direct.
      if (!Array.isArray(bans) && Array.isArray(bans?.bans)) {
        bans = bans.bans;
      }

      res.json(Array.isArray(bans) ? bans : []);
    } catch (err) {
      console.error(err);
