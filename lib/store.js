...
class TenantManager {
  ...
  // première connexion réussie via /login/<slug> (voir claimTenant).
  async createTenantWithApp({ name, clientId, clientSecret }) {
    await ensureSchema();
    const slug = await this.generateUniqueSlug(name || clientId);
    const [result] = await pool.query(
