...
class TicketManager {
  ...
  saveTickets() {
    try {
      const data = [...this.tickets.values()];
  ...
  async reconnectTickets() {
    try {
      const staffGuild = await this.client.guilds.fetch(process.env.STAFF_GUILD_ID);
    ...
    for (const [id, ticket] of this.tickets.entries()) {
      try {
        ...
        try {
          const ticketType = TICKET_TYPES.find((t) => t.id === ticket.typeId);
          const allowedRoles = ticketType ? ticketType.allowedRoles : ALL_STAFF_ROLES;

          // Personne (par défaut) ne voit le salon
          await channel.permissionOverwrites.edit(channel.guild.roles.everyone.id, {
            ViewChannel: false,
          });

          // On retire l'accès à tous les rôles staff connus qui ne sont pas
          // autorisés pour ce type de ticket (au cas où la config a changé)
          for (const roleId of ALL_STAFF_ROLES) {
          ...
          // On accorde l'accès aux rôles autorisés pour ce type de ticket
  ...
  async createTicket(user, ticketType, sourceGuild) {
    ...
    // Rôles autorisés à voir ce type de ticket précis (défini dans config.js)
    const allowedRoles = ticketType.allowedRoles || [];

    const permissionOverwrites = [
      {
        // Personne ne voit le salon par défaut (@everyone bloqué)
      ...
      // Seuls les rôles listés dans allowedRoles pour ce type de ticket
      // obtiennent explicitement l'accès. Tout rôle non listé reste bloqué.
    ...
    if (process.env.STAFF_CATEGORY_ID) {
      channelOptions.parent = process.env.STAFF_CATEGORY_ID;
    }
    ...
    try {
      ...
      if (panelMsg) {
        const disabledRow = new ActionRowBuilder().addComponents(
          ...panelMsg.components[0].components.map((c) =>
  ...
  // ─────────────────────────────────────────────────────────
  // RELAY : Salon Staff → User DM
  // Chaque message staff (y compris les replies) est relayé
  // avec le pseudo du staff affiché dans l'embed.
  // ─────────────────────────────────────────────────────────
