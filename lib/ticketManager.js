...
class TicketManager {
  ...
  async loadTickets() {
    try {
      await ensureSchema();
      const [rows] = await pool.query("SELECT data FROM tickets WHERE status = 'open'");
  ...
  async saveTicket(ticket) {
    try {
      await pool.query(
        `INSERT INTO tickets (id, data, status, created_at) VALUES (?, ?, ?, ?)
         ON DUPLICATE KEY UPDATE data = VALUES(data), status = VALUES(status)`,
        [ticket.id, JSON.stringify(ticket), ticket.status, ticket.createdAt || Date.now()]
      );
    } catch (err) {
  ...
  async reconnectTickets() {
    ...
    const toRemove = [];
    const allKnownRoles = getAllKnownRoles();
  ...
  async createTicket(user, ticketType, sourceGuild) {
    ...
    const { staffGuildId, staffCategoryId } = store.getBot();
