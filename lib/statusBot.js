...
// Anti-spam : cf. botManager.js — délai minimal entre deux restart(), et
// détection du rate limit Discord (429) pour ne jamais retenter pendant
// le délai qu'il impose.
...
function extractRetryAfterMs(err) {
  ...
  const candidates = [err?.retryAfter, err?.retry_after, err?.rawError?.retry_after, err?.data?.retry_after, err?.timeToReset];
...
class StatusBotController extends EventEmitter {
  ...
  async start() {
    ...
    if (this._rateLimitedUntil > Date.now()) {
      ...
      this.status = 'error';
      this.lastError = `Discord a temporairement bloqué les connexions pour ce bot (HTTP 429 - rate limit). Nouvelle tentative automatique dans ${waitSec}s.`;
      this.emit('statusChanged', this.getStatus());
      return this.getStatus();
    }
    ...
    try {
      ...
      if (!this._loginSettled) {
        ...
        if (retryAfterMs) {
          ...
          this.status = 'error';
          this.lastError = `Discord bloque temporairement les connexions pour ce bot (HTTP 429 - rate limit). Nouvelle tentative automatique dans ${waitSec}s.`;
          console.error(`⏳ [tenant ${this.tenantId}] Bot status rate limité par Discord (429). Attente ${waitSec}s.`);
        } else {
          this.status = 'error';
  ...
  async restart() {
    ...
    if (this._lastRestartAt && elapsed < MIN_RESTART_INTERVAL_MS) {
      ...
      console.warn(`⏳ [tenant ${this.tenantId}] restart() bot status ignoré (anti-spam), réessaie dans ${waitSec}s.`);
