...
// 1. Détection des variables d'environnement (Support de DB_* configuré sur Render, MYSQL_* et URL)
const dbUrl = process.env.MYSQL_URL || process.env.DATABASE_URL;

let connectionConfig;

if (dbUrl) {
  // Option via URL complète
  ...
  // Option via variables individuelles (Lectures de vos variables Render : DB_HOST, DB_USER, etc.)
  ...
  connectionConfig = {
    ...
    password,
    database,
    // Requis pour Aiven MySQL : force le SSL si non désactivé explicitement
...
// Vérification de sécurité dans les logs
if (!connectionConfig.host || connectionConfig.host === 'localhost' || connectionConfig.host === '127.0.0.1') {
  console.warn('⚠️ ATTENTION : Aucune variable MySQL valide détectée, tentative sur localhost...');
}
