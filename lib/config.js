...
// Le champ "Icône" des types de ticket est un texte libre saisi dans le
// dashboard : rien n'empêchait d'y coller un caractère qui n'est pas un
// emoji Discord valide (texte, ZWJ cassé, sélecteur de variante seul...).
// Discord rejette alors l'ENSEMBLE de l'envoi du panel avec une erreur
// "COMPONENT_INVALID_EMOJI" — un seul type de ticket cassé empêchait donc
// tout le panel de partir. On valide ici en amont (à la sauvegarde) et on
// s'en sert aussi comme filet de sécurité au moment de construire les
// boutons (voir bot.js `_buildTypeRows`).
const CUSTOM_EMOJI_RE = /^<a?:\w{2,32}:\d{17,20}>$/;
// Couvre : un emoji unicode "standard" (avec sélecteur de variante optionnel
// \uFE0F) ou une séquence liée par ZWJ (ex: 👨‍👩‍👧). Volontairement permissif
// plutôt que parfait — le but est de bloquer le texte brut, pas de valider
// exhaustivement chaque séquence Unicode existante.
